...
## Работа браузера (Chromium) 🌐

После установки драйвера `hid-multitouch` и загрузки модуля тачскрин работает **нативно**:

- **Навигация** — касания, скролл, тапы по ссылкам — всё корректно сразу 🎯
- **Долгий тап** — Chromium сам обрабатывает его как контекстное меню, без каких-либо настроек
- **ЛКМ** — одиночное касание работает как клик
- **Скролл** — свайп по странице работает плавно

Никаких дополнительных флагов Chromium (`--touch-events`, `--force-renderer-accessibility`) для базовой работы тача **не требуется**. Флаг `--force-renderer-accessibility` нужен только для авто-показа Onboard (см. ниже).

> ⚠️ Важный момент: если установлен скрипт `touch-rightclick.py` для эмуляции ПКМ на рабочем столе — в нём есть встроенная проверка активного окна (`is_browser_active()`). Когда фокус на Chromium, скрипт **пропускает** эмуляцию ПКМ, чтобы не было двойного срабатывания с родным контекстным меню браузера. Подробнее — в разделе «Настройка кликов».
...
### Защита от конфликта с браузером 🛡️

В скрипте есть встроенная проверка активного окна — функция `is_browser_active()`:

- Если в фокусе **Chromium / Chrome / Firefox** — скрипт **не эмулирует ПКМ**, так как браузер сам обрабатывает долгий тап как контекстное меню
- Скрипт проверяет `WM_CLASS` активного окна через `xprop`
- В логе появляется: `⏭ Пропуск ПКМ — активен браузер`
- На рабочем столе (XFCE, GNOME, файловый менеджер) — ПКМ работает как обычно

Это гарантирует, что в браузере не будет двойных контекстных меню.
...
## Установка GNOME (Xorg) 🖥️

GNOME полностью работоспособен на OPI Zero 3W (PowerVR GPU, Xorg).
...
После установки в LightDM появится три сессии:
- `GNOME` — Wayland (не работает, драйверы PowerVR только под Xorg) ❌
- `GNOME (Xorg)` — классический GNOME на X11 ✅ **выбирать этот**
- `GNOME Classic` — GNOME Flashback, минимальная анимация

### Выбор сессии

При входе в LightDM нажми на иконку с шестерёнкой ⚙️ (обычно рядом с полем пароля) и выбери **«GNOME (Xorg)»**.
...
После перезагрузки система автоматически войдёт в GNOME (Xorg).

### Совместимость

| Компонент | Статус |
|-----------|--------|
| GNOME Shell (Xorg) | ✅ Плавно, без глюков |
| Тачскрин (ЛКМ/ПКМ) | ✅ Работает (скрипт в ~/.config/autostart/ подхватывается и GNOME) |
| Onboard | ✅ Работает |
| Встроенная клавиатура GNOME (свайп снизу) | ✅ Работает |
| Анимации, обои, расширения | ✅ Всё штатно |
